...
#### R2 fix — single-token append-prefill guard (patch `v080-0004`, landed 2026-06-10)
Phase 3b empirically found the ONE R2 hole the byte-identical multi-token proof missed: a prefill chunk of **exactly 1 token landing on a non-empty KV cache** mis-samples (argmax can flip; max-abs-diff ~2e+01). **Root cause is NOT in `runBaseModelPrefillChunk`** — it is `deduceModeVanilla` in `cpp/plugins/attentionPlugin/attentionPlugin.cpp:93-110`, which keys chunked-prefill vs vanilla-decode SOLELY on `qInputTensor.getShape()[1]`: non-empty `kvcache_start_index` + `qSeqLen==1` → `kVANILLA_DECODING`. So a 1-token append-prefill is misrouted to the decode path, where RoPE position comes from `contextLength` (`launchApplyRopeWriteKV(..., contextLengthTensor, ...)` → `posStartId = kvCacheEndLens[b] - qSeqLen` in `cpp/kernels/posEncoding/applyRopeWriteKV.cu:150`). The chunk path sets `contextLength = chunkLen = 1`, so the RoPE position collapses to **0 (chunk-relative)** instead of the absolute `priorKV+0`, and XQA attends over `sequence_lengths=1` instead of the full KV. The chunked-prefill branch instead uses absolute `kvCacheEndIdxs = kvCacheStartIdx + contextLength`. The plugin boundary has **no shape-distinct signal** (a genuine decode and a 1-token prefill-append are both `qSeqLen==1` with non-empty KV), and a 1-token *intermediate* chunk is equally affected (bakes a wrong RoPE position into committed KV).
**Fix (runtime invariant, not a plugin/engine ABI change):** in `AsrStreamingSessionRuntime::appendChunkImpl`, carry the trailing token of a non-final **text** chunk forward into the next chunk, so every step after the first (and the final step) packs ≥ 2 tokens → stays on the chunked-prefill path. First chunk runs on empty KV (`kNORMAL_PREFILL`, valid at length 1) → never deferred. Carry-over is text-only (audio chunks bind their own per-chunk embeds and are never length 1); an audio 1-token step on non-empty KV is hard-refused. Carry state commits only after the prefill succeeds (refusal-safe). **One-shot path untouched → byte-identical (re-verified).** Spike m1 R3 relaxed to accept the coalesced intermediate boundary (`kvB1 ∈ {splitAt-1, splitAt}`; final `kvB2 == totalTokens` unchanged). Gate (orin-nx engines-v080): N-1 splits 30/29, 40/39, 64/63, 100/99 now argmax MATCH + diff **0.0**; multi-token splits still 0.0; m2 PASS x3. **R2 parity now FULLY green for all final-chunk sizes → Phase 4 unblocked.**
