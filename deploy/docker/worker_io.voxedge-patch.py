        ...
        try:
            ...
            for line in self._proc.stdout:
                ...
                with self._inflight_lock:
                    ...
                    # Robustness: a worker error event without a routable id
                    # would otherwise be dropped, hanging the caller the full
                    # 60s q.get timeout. At N=1 (a single inflight request)
                    # route an unkeyed/unmatched error to the sole queue so the
                    # caller surfaces the worker error fast instead of timing
                    # out. Only errors are rerouted (never partial/done) to
                    # avoid corrupting a legitimately stale stream.
                    if (
                        q is None
                        and event.get("event") == "error"
                        and len(self._inflight) == 1
                    ):
                        q = next(iter(self._inflight.values()))
                if q is not None:
                    q.put(event)
