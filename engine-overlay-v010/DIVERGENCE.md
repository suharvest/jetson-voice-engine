# voxedge-engine v0.10 divergence ledger

```text
upstream:       github.com/NVIDIA/TensorRT-Edge-LLM (Apache 2.0)
UPSTREAM_PIN:   71dd1bae032e70771265917ec74d3ff4cad07a10   (= tag v0.10.0, pure NVIDIA)
upstream set:   upstream-v010-prs/series (4 rebased commits; PR #118/#146/#149)
local series:   v010-candidate/series (32 product patches; replay complete)
state:          see patches/PATCH-STATE-v010.md
```

> This scaffold's active chain is `upstream-v010-prs/series` plus
> `v010-candidate/series`. The copied v0.9.1 product directory is history only.
> The integrated 4+32 chain passes clean forward and reverse replay against the
> official v0.10.0 tree. Device compile, regenerated engines, and runtime tests
> remain release gates.
>
> Upstream replacement ledger:
>
> - PR #118: explicit CUDA architectures (retained) plus only the missing
>   static-target INTERFACE `--wrap=_cudaLaunchKernelEx` propagation (retained);
> - PR #145: pre-TRT-10.8 FP4 guard (retired: v0.10.0 contains it);
> - PR #146: linear+MRoPE normalization (retained, rebased);
> - PR #147: pre-TRT-10.7 stream reader (retired: v0.10.0 contains it);
> - PR #148: mask-scoped FMHA cubin load (retired: v0.10.0 removes the legacy
>   backend and replaces it with CuTe DSL FMHA-v2);
> - PR #149: destination checkpoint dtype (retained, rebased).
>
> The v0.10 CuTe patch intentionally does not duplicate upstream's existing
> static-library shim link. It only adds the missing static-target INTERFACE
> wrap option. Product link/runtime validation is pending Orin hardware qualification.
>
...
pr_candidate: false
upstream_pr:  —
rationale:    MOSS-TTS-Nano 产品模型移植 (linear-attention KV kernel + stateful runtime + JSON-line worker)。上游无此模型。codec decode 与 voice-clone encoder 共用显式 codec 目录，不再要求 engine root 复制 codec plan/meta。TTFA 157ms, 19× faster than ORT。最干净一类: 几乎全 ISO 新文件, 仅 1 处 CMake 注册算 CORE
