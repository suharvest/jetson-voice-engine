...
path = Path(sys.argv[1])
text = path.read_text()
old = 'checksums = "patches/upstream-v010-prs/SHA256SUMS"'
assert text.count(old) == 1
path.write_text(text.replace(
    old, 'checksums = "patches/upstream-v010-prs/MISSING"'))
PY
...
python3 - "${TMP_ROOT}/lock-order-overlay/patches/upstream-v010-prs/LOCK" <<'PY'
from pathlib import Path
import sys
...
python3 - "${TMP_ROOT}/sums-order-overlay/patches/upstream-v010-prs/SHA256SUMS" <<'PY'
from pathlib import Path
import sys
...
for relative in \
  engine-overlay-v010/patches/upstream-v010-prs/series \
  engine-overlay-v010/patches/upstream-v010-prs/LOCK \
  engine-overlay-v010/patches/upstream-v010-prs/SHA256SUMS \
  engine-overlay-v010/patches/v010-candidate/series \
  engine-overlay-v010/patches/v010-candidate/SHA256SUMS; do
  cmp "${INNER_ROOT}/${relative}" "${AUTOCLONE}/${relative}"
done
while IFS= read -r file; do
  cmp "${INNER_ROOT}/engine-overlay-v010/patches/upstream-v010-prs/${file}" \
    "${AUTOCLONE}/engine-overlay-v010/patches/upstream-v010-prs/${file}"
done < "${HERE}/patches/upstream-v010-prs/series"
while IFS= read -r file; do
  cmp "${INNER_ROOT}/engine-overlay-v010/patches/v010-candidate/${file}" \
    "${AUTOCLONE}/engine-overlay-v010/patches/v010-candidate/${file}"
done < "${HERE}/patches/v010-candidate/series"
check_sums "${AUTOCLONE}/engine-overlay-v010/patches/upstream-v010-prs"
check_sums "${AUTOCLONE}/engine-overlay-v010/patches/v010-candidate"

if [ -n "${OFFICIAL_CHECKOUT}" ]; then
  gitlink_count="$(git -C "${OFFICIAL_CHECKOUT}" ls-tree -r "${PIN}" \
    | awk '$1 == "160000" {count++} END {print count + 0}')"
  [ "${gitlink_count}" -ge 0 ] \
    || { echo "ERROR: expected three v0.10.0 gitlink fixtures, found ${gitlink_count}" >&2; exit 1; }
  ...
  while IFS='|' read -r file pr commit parent tree patch_id expected_sha; do
    ...
    git -C "${OFFICIAL_CLONE}" fetch "${OFFICIAL_CHECKOUT}" "${commit}" \
      >/dev/null
  done < "${HERE}/patches/upstream-v010-prs/LOCK"
...
PY
  ...
  bash "${AUTOCLONE}/engine-overlay-v010/tests/verify-patch-stack.sh" \
    "${OFFICIAL_CHECKOUT}" >/dev/null
else
  bash "${AUTOCLONE}/engine-overlay-v010/tests/verify-patch-stack.sh" >/dev/null
fi
