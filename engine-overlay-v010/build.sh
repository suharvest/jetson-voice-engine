...
#
# CONTRACT
#   inputs : UPSTREAM_PIN, upstream.remote, patches/upstream-v010-prs/,
#            addon/, patches/v010-candidate/,
#            a build manifest
...
#   2. verify + apply the 4 exact proposed-upstream commits
#   3. copy addon/ over the checkout (new files, exec bits preserved)
#   4. validate + apply the explicit 32-patch local product series
...
#   EDGELLM_UPSTREAM_REMOTE=/home/harvest/project/edgellm-v010 bash build.sh
...
UPSTREAM_PATCH_DIR="${HERE}/patches/upstream-v010-prs"
...
load_series "${UPSTREAM_PATCH_DIR}" "${UPSTREAM_PATCH_DIR}/series" 4 "proposed-upstream"
...
# v0.8/v0.9/v0.9.1 patch files remain on disk solely as rollback/history.
echo "==> applying local product patches"
PATCH_DIR="${HERE}/patches/v010-candidate"
[ -f "${PATCH_DIR}/series" ] \
  || { echo "ERROR: missing v0.10 product patch series" >&2; exit 5; }
if command -v sha256sum >/dev/null 2>&1; then
  (cd "${PATCH_DIR}" && sha256sum -c SHA256SUMS)
else
  (cd "${PATCH_DIR}" && shasum -a 256 -c SHA256SUMS)
fi
load_series "${PATCH_DIR}" "${PATCH_DIR}/series" 32 "local-product"
LOCAL_PATCHES=("${SERIES[@]}")
for p in "${LOCAL_PATCHES[@]}"; do
  apply_one "${p}"
done
echo "==> patched source tree ready at ${WORKDIR}"
echo "    v0.10.0 base (71dd1bae) + 4 exact proposed-upstream commits +"
echo "    32 sparse local product patches (rebased candidate)."
echo "    streaming worker + slot-pool +"
echo "    shared-engine ctors + native voice clone/language conditioning +"
echo "    SparkTTS mixed-precision/int4 opt-ins + MOSS (in-series)."
echo "    See patches/PATCH-STATE-v010.md for dispositions."
...
python3 "${HERE}/verify-release-target.py" \
  ...
  --tensorrt "${TRT_ACTUAL}"
# ============================================================================
# CUTE DSL CONFIGURATION (v0.10.0 migration):
#
# (A) VOICE WORKER BUILDS (this script's default): ENABLE_CUTE_DSL=fmha.
#     v0.10 always compiles the new CuTe FMHA-v2 runner. OFF therefore cannot
#     produce the plugin from a pristine source archive because its generated
#     cutedsl_all.h is absent. The talker MLP still uses our sm_87 fallback
#     kernels, but the common attention plugin requires the FMHA artifact.
#
...
#          the device-qualified nvidia-cutlass-dsl 4.6.1 toolchain and
#          cupy-cuda12x 12.3.0, then:
#          python kernelSrcs/build_cutedsl.py --kernels fmha \
#            --gpu_arch sm_87 --arch aarch64 --cuda-version 12
#          (the packaged historical SM87 archive was built with CUDA 13.2 and is
...
# port/v090-workers and are now compiled against the active v0.10.0 tree.
...
# reference only — do not use it on the v0.10.0 chain.
...
echo "    N=2 runtime: select the eventual jetson-edgellm-v010-n2 profile (stream_mode=worker,"
